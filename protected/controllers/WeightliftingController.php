...
class WeightliftingController extends Controller
{
	public $halloffame;
	public $secretaries;
	public $captains;
	...
	public function actionContact()
	{
		...
		$this->coach = $connection->createCommand()->SELECT('*')->FROM('coach')->queryAll();
		//$this->convener = $connection->createCommand()->SELECT('*')->FROM('convener')->queryAll();
	...
	// {
		...
		// else {
		// $check=$_GET["sport"];
			// $people = $connection->createCommand()->SELECT('*')->FROM('quest')->WHERE("Team LIKE '$check'")->ORDER("Name ASC")->queryAll();
